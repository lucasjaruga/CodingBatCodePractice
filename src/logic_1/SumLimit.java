...
class SumLimit {
	public int sumLimit(int a, int b) {
		  int sum = a + b;
		  int noDigitsInA = String.valueOf(a).length();
		  int noDigitsInSum = String.valueOf(sum).length();
		  
		  boolean areDigitsInA_LessThenInSum = noDigitsInA < noDigitsInSum;
		  
		  if(areDigitsInA_LessThenInSum) {
			  return a;
		  } else {
			  return sum;
		  }
